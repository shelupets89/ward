...
- **A formula cannot put anything in `/Applications`.** `post_install` runs sandboxed and `/Applications` is not on the allowlist (`Errno::EPERM`, observed), and `Keg.keg_link_directories` is `bin etc include lib sbin share var` — no linking mechanism reaches it. `bin` *is* linked, so the step lives there instead: `packaging/homebrew/ward` installs as `bin/ward`, writes the symlink itself, and `caveats` just says to run `ward`. It still cannot happen during the install.
...
- **No `precondition` or `fatalError` on a developer constant, in any feature.** `AppDelegate.features` constructs every controller before `applicationDidFinishLaunching` runs a single `recoverLeakedState()`, so a trap while building *any* feature — including a fail-open one — kills the launch that would have offered to clear a `pmset disablesleep` a crash left behind. Fail-closed features have the same problem later, mid-session, once the setting is already flipped. Where a bad value gets silently substituted, `assert` beside the substitution is the developer signal (`ExpiryCheckInterval.clamped`, `EscapeHoldTracker.clampedHoldDuration`). Where it is honoured as-is and the outcome is already safe, define that outcome and test it rather than asserting against it (`KeepAwakeSession`) — an `assert` there buys a debug-only signal at the cost of the test that would prove the claim, and it traps the whole test run if you try.
