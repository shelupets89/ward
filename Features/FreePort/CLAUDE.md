...
- **Classify the final report by what was *done*, never by what is signalable now.** `Stage.afterForceKill` carries `forceKilledTargets` — the actual SIGKILL batch, handed over by the caller. Re-deriving it as "approved and present in the current snapshot" was the root of a bug that outlived three rounds of fixes: a process that closes its listening socket while handling SIGTERM and rebinds afterwards is missing when the batch is built, so it is never signalled, yet is back on the port when the result is read — indistinguishable, under that rule, from one that took a SIGKILL and lived. `stillHeld` then partitions every holder three ways (`signalled`, `undelivered`, `untouched`) so nothing can be silently omitted; "show everything still on the port" is the post-kill half of "show everything about to die". `undeliveredTargets` is the SIGKILL round's result **only**: a pid refused at SIGTERM whose SIGKILL then landed *was* signalled.
