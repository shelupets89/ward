...
- **A refused signal is not a survived one.** `kill(2)` can return `EPERM` even for a process this user owns (macOS protects some). The refusal is carried from `ProcessSignaller` back to `KillEscalation.Stage.afterForceKill(refusedTargets:)` for one reason: without it the process reappears in the next snapshot and gets reported as having "survived both SIGTERM and SIGKILL", describing a delivery that never happened. Don't drop that thread to simplify the signature.
