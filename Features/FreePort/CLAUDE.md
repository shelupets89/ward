...
- **The final report accounts for every holder, not a subset.** `stillHeld` carries three lists — `signalled` (outlived a delivered SIGKILL), `undelivered` (macOS refused, or the call failed), `untouched` (never approved, never signalled). It is a partition of `snapshot.holders`, so nothing can be silently omitted. Three separate review rounds each found a different shape here that reported a subset and dropped the rest; "show everything still on the port" is the post-kill half of "show everything about to die". `undeliveredTargets` is the SIGKILL round's result **only**: a pid refused at SIGTERM whose SIGKILL then landed *was* signalled, and folding the earlier refusal in would report it as untouched.
